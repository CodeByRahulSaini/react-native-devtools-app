...
const RUNTIME_GLOBAL = '__FUSEBOX_PLUGINS_DISPATCHER__';
...
/**
 * PluginsBindingsModel - Handles CDP communication with the plugins dispatcher.
 *
 * This model communicates with `__FUSEBOX_PLUGINS_DISPATCHER__` in the React Native app
 * to route messages between DevTools panels and their corresponding plugins.
 *
 * Communication flow:
 * 1. DevTools calls Runtime.addBinding(BINDING_NAME) to receive messages from plugins
 ...
 */
export class PluginsBindingsModel extends SDK.SDKModel.SDKModel<EventTypes> {
  ...
  private bindingCalled(event: BindingCalledEventTargetEvent): void {
    ...
    if (parsedMessage) {
      ...
      if (this.fuseboxDispatcherIsInitialized) {
        ...
        // But PluginsBindingsModel is busy executing async tasks
  ...
  private dispatchMessageToDomainEventListeners(domainName: DomainName, message: JSONValue): void {
    ...
    if (errors.length > 0) {
      throw new AggregateError(
        errors,
        `Error occurred in PluginsBindingsModel while calling event listeners for domain ${domainName}`,
      );
    }
  }
  ...
  async initializeDomain(domainName: DomainName): Promise<void> {
    const runtimeModel = this.target().model(SDK.RuntimeModel.RuntimeModel);
    if (!runtimeModel) {
      throw new Error(`Failed to initialize domain ${domainName} for PluginsBindingsModel: runtime model is not available`);
    }
  ...
  async sendMessage(domainName: DomainName, message: JSONValue): Promise<void> {
    ...
    const runtimeModel = this.target().model(SDK.RuntimeModel.RuntimeModel);
    if (!runtimeModel) {
      throw new Error(`Failed to send message from PluginsBindingsModel for domain ${domainName}: runtime model is not available`);
    }
  ...
  async enable(): Promise<void> {
    if (this.enabled) {
      throw new Error('PluginsBindingsModel is already enabled');
    }

    const runtimeModel = this.target().model(SDK.RuntimeModel.RuntimeModel);
    if (!runtimeModel) {
      throw new Error('Failed to enable PluginsBindingsModel: runtime model is not available');
    }
    ...
    await this.waitForFuseboxDispatcherToBeInitialized()
      ...
      .then(response => {
        if (response.exceptionDetails) {
          throw new Error('Failed to get binding name for PluginsBindingsModel on a global: ' + response.exceptionDetails.text);
        }

        if (response.result.value === null || response.result.value === undefined) {
          throw new Error('Failed to get binding name for PluginsBindingsModel on a global: returned value is ' + String(response.result.value));
        }

        if (response.result.value === '') {
          throw new Error('Failed to get binding name for PluginsBindingsModel on a global: returned value is an empty string');
        }
      ...
      .then(response => {
        ...
        if (possiblyError) {
          throw new Error('Failed to add binding for PluginsBindingsModel: ' + possiblyError);
        }
  ...
  private initializeExecutionContextListeners(): void {
    const runtimeModel = this.target().model(SDK.RuntimeModel.RuntimeModel);
    if (!runtimeModel) {
      throw new Error('Failed to initialize execution context listeners for PluginsBindingsModel: runtime model is not available');
    }
  ...
  private async waitForFuseboxDispatcherToBeInitialized(attempt = 1): Promise<void> {
    ...
    const runtimeModel = this.target().model(SDK.RuntimeModel.RuntimeModel);
    if (!runtimeModel) {
      throw new Error('Failed to wait for Plugins dispatcher initialization: runtime model is not available');
    }
    ...
    await runtimeModel.agent.invoke_evaluate({
      ...
      .then(response => {
        if (response.exceptionDetails) {
          throw new Error('Failed to wait for Plugins dispatcher initialization: ' + response.exceptionDetails.text);
        }
...
SDK.SDKModel.SDKModel.register(PluginsBindingsModel, {capabilities: SDK.Target.Capability.JS, autostart: false});
